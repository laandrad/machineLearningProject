...
The raw data can be efficiently downloaded from the internet using R's *sqldf* package. The raw dataset contains 19622 observations across 160 variables.
```{r downloadData}
...
### Explatory analysis of activity separation by features
...
A Principal Components Analysis (PCA) is run with the training data to reduce the number of features to train the learner algorithm. A PCA decomposes the variance of the original matrix in decreasing order so that the most variance is contained in the first few components. The *pca* algorithm included in the *caret* package selects keeps only 18 components to capture 90% of the variance of the original 52 numeric variables. To further reduce the number of feature is to get rid of near zero variance features. However, there is no near zero variance features in the PCA matrix. 
```{r pca}
...
## Model Training
A Random Forest (RF) algorithm is selected to train the learner using the parallel implementation in the *ParallelForest* package in R. To optimize the parameters, a 10-fold cross-validation method is used.
```{r model}
library(doParallel)
